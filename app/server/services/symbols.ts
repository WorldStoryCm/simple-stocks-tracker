...
async function listPaged(userId: string, input: SymbolListInput) {
...
async function update(userId: string, input: SymbolUpdateInput) {
  const [symbol] = await db.update(symbols)
    ...
    .where(and(eq(symbols.id, input.id), eq(symbols.userId, userId)))
    .returning();
  return symbol;
}

async function enrichAll(userId: string) {
